...
def sendToFile():
    try:
        if os.path.getsize('./admin/readadmin/fileAdmin1.txt'):
            print("File 'fileAdmin1.txt' exist !")
    except FileNotFoundError as infofile :
        print("File : 'fileAdmin1' does not exist !", infofile)
        print("File : 'fileAdmin1' created !")
    finally:
        with open('./admin/readadmin/fileAdmin1.txt', 'a+') as file:
            file.write(str("Date : "))
...
buttonName = Button(text='Save', width=10, fg='yellow', bg='navy',
    bd=3, highlightbackground='#82193e', activebackground='dark turquoise',
    command=sendToFile)
