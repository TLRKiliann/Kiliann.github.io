...
if __name__ == '__main__':
    ...
    class Control:
        ...
        def print_selected_date(self):
            print(self.data)
            try:
                if os.path.getsize('./patient_agenda/events3/patient_3_calendar.txt'):
                    print("+ File 'patient_3_calendar.txt' exist !")
                    file=open('./patient_agenda/events3/patient_3_calendar.txt','wb')
                    dump(self.data, file)
                    file.close()
                    subprocess.call('./patient_agenda/events3/entrer_event3.py')
            except FileNotFoundError as pret:
                    print("+ File not existing!", pret)
                    print("+ File 'patient_3_calendar.txt' created !")
                    file=open('./patient_agenda/events3/patient_3_calendar.txt','wb')
                    dump(self.data, file)
                    file.close()
                    subprocess.call('./patient_agenda/events3/entrer_event3.py')
