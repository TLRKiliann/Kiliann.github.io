...
class Application(Frame):
    ...
    def lienDirect(self):
        ...
        """
        try:
            if os.path.getsize('./histv/doc_histv7/Hvie_patient1.txt'):
                print("+ File 'Story Life' exist (add)!")
                subprocess.call('./histv/doc_histv7/story_write.py')
        except FileNotFoundError as outmsg:
            print("+ Sorry, file 'Story Life' not exist !", outmsg)
            print("+ File 'Story Life' created !")
            with open('./histv/doc_histv7/Hvie_patient1.txt', 'w') as file:
                file.write(".")
            self.confRec()
